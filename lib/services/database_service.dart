...
//helps to check if a task is complete or not for a particular user
  Future<bool?> toggle(Database db, int id, int userId) async {
    ...
    if (result.isNotEmpty) {
      ...
      return completedValue ==
          1; // Return true if completedValue is 1, else false
    }
    return null; // Return null if the task with the specified ID isn't found
  }

  @override
  Future createTodo(String description, String createdDate, String startTime,
      bool completed, int userId) async {
    try {
  ...
  @override
  Future<List<Map<String, dynamic>>?> getTasksOrderedByDate(
  ...
  //function which permits to delete a task from the database
  @override
  Future deleteTodo(userId) async {
    try {
      final db = await initialize();

      final delete =
          db.delete('todo', where: ' userId = ?', whereArgs: [userId]);
      return delete;
    } catch (e) {
      print('$e');
    }
